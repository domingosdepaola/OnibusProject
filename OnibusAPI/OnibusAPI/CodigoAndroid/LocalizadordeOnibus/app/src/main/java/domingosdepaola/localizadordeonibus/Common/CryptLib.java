...
public class CryptLib {
    ...
    public static final String IV = "DomingosDePaola";
    ...
    public String decrypt(String value, String key) throws IllegalBlockSizeException, BadPaddingException, InvalidAlgorithmParameterException, InvalidKeyException, UnsupportedEncodingException, NoSuchAlgorithmException {

        return this.decrypt(value, key, CryptLib.IV);
    }

    public String encrypt(String value, String key) throws IllegalBlockSizeException, BadPaddingException, InvalidAlgorithmParameterException, InvalidKeyException, UnsupportedEncodingException, NoSuchAlgorithmException {

        String crypto = this.encrypt(value, CryptLib.SHA256(key, CryptLib.DEFAULT_SIZE), CryptLib.IV);

        return crypto.substring(0, crypto.length() - 1);
    }
    ...
    private String encryptDecrypt(String _inputText, String _encryptionKey,
        ...
        if (_mode.equals(EncryptMode.DECRYPT)) {
            ...
            byte[] decodedValue = Base64.decode(_inputText.getBytes(),0,_inputText.getBytes().length,
                    Base64.DEFAULT);
            byte[] decryptedVal = _cx.doFinal(decodedValue); // Finish
            // multi-part
            // transformation
            // (decryption)
            _out = new String(decryptedVal).trim();
        }
    ...
    public String decrypt(String _encryptedText, String _key, String _iv)
        ...
        return encryptDecrypt(_encryptedText, CryptLib.SHA256(_key, CryptLib.DEFAULT_SIZE), EncryptMode.DECRYPT, _iv);
    }
